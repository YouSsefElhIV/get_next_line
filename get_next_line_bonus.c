...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/14 19:01:56 by yel-haya          #+#    #+#             */
/*   Updated: 2024/11/18 15:25:23 by yel-haya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_first_line(int fd, char first_line[fd])
{
	char	*buf;
	size_t	n_bytes;
	char	*new_first_line;
...
{
	static char	*first_line[10240];
	char		*next_line;

	if (fd < 0 || fd >= 10240 || BUFFER_SIZE >= 2147483647)
		return (NULL);
	...
	first_line[fd] = get_first_line(fd, first_line[fd]);
	if (!first_line[fd])
		return (NULL);
	next_line = get_other_line(&first_line[fd]);
	if (!next_line)
		return (free(first_line[fd]), first_line[fd] = NULL, NULL);
	if (ft_strlen(next_line) == 0)
	{
		free(first_line[fd]);
		return (free(next_line), first_line[fd] = NULL, NULL);
	}
	return (next_line);
}
